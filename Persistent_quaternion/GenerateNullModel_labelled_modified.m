...
%radius=load('ParameterForRandomModel_basedon_centroid.dat')
radius=load('ParameterForRandomModel_basedon_nuc.dat');

load('MakeListColumnarStructurePrediction/DT_E185_nuclei/centroid_and_surface_cells.mat','centroid');


[~,LCC]=readClusterFile('beforeFusion_zone80.dat');
disp('LCC')
length(LCC)
    ...
    for k=1:length(radius)
        ...
        while flag
            ...
            if (R(k,1)>=0)&(R(k,1)<=1)
                 ...
                 cellid=Search_cell_in_approximity(X,Y,Z,radius(i,2),c,DV,ML,PDaxis);
                 size_of_cluster=length(LCC{k});
                  if (length(cellid)-size_of_cluster)>=5
                        if R(k,1)>0.75
                            cutoff=22;
                        else
                            cutoff=12;
