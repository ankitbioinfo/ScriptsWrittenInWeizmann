...
for i=1:Row
    ...
    for j=1:Col
        ...
        if chro<6
            ...
            set(gca,'fontsize',11);
            axis([0,1,0,1.15*10^-3])
            %title(strcat(titlename{bonetype},', Cell(-), Nuclei(--)'),'fontweight','normal','fontsize',14)
          
            hl=legend(q,legendarray,'location','northeast','fontsize',5);
            legend 'boxoff';
...
saveas(h1,['Individual_Profile_Delaunay']);
saveas(h1,['Individual_Profile_Delaunay.png']);
