...
fcelallcolor={'r','g','b','b','b','ro'};
fnucallcolor={'r','g','b','b','b','b^'};
...
for i=1:Row
    ...
    for j=1:Col
        ...
        nucstd=(nanstd(nuc'))';
        if chro==3
                    tnuc=nuc(:,[1,3]); tnucstd=(nanstd(tnuc'))';
                    nucstd(1:3,:)=tnucstd(1:3,:);
                    nucavg(1:3,:)=mean(tnuc(1:3,:),2);
        end
...
saveas(h1,['wildType_Together_nuclei']);
saveas(h1,['wildType_Together_nuclei.png']);
