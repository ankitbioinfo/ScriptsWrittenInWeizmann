...
%               text(0.01,5.0+0.1*diff(gap),strcat('p=',sprintf('%0.1e',pR)));
...
%               text(ind3(1)/50,5.0+0.1*diff(gap),strcat('p=',sprintf('%0.1e',pH)));
...
%               plot([ind3(1)/50,ind3(1)/50],[0,11],'k--');
              ...
              if pR<0.001
                  text(ind2(1)/50,value+0.09*diff(gap),strcat('p=',sprintf('%0.1e',pR)));
              else
                  text(ind2(1)/50,value+0.09*diff(gap),strcat('p=',sprintf('%0.3f',pR)));
              end
...
%              [h,pE]=kstest2(celavg,nucavg);
%             text(0.1,7,sprintf('%0.3f',pE));
...
saveas(h1,['AverageProfileSurfaceArea']);
saveas(h1,['AverageProfileSurfaceArea.png']);
