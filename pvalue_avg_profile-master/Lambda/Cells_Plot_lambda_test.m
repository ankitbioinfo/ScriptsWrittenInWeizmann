...
titlename={'DT WT Cell','PT WT Cell','DT MT Cell','PT MT Cell'};
...
h1=figure();
XL=0.07;XR=0.03;XGap=0.05;Row=3;
YT=0.06;YB=0.08;YGap=0.07;Col=4;
...
for i=1:Row
    ...
    for j=1:Col
        ...
        p(1)=plot(myinterval,celavg,strcat('r','-'),'linewidth',0.2);
        legendarray2{1}=tname{i};
        hold on 
      
        p(2)=plot(myinterval,nucavg,strcat('b','--'),'linewidth',0.2);
        legendarray2{2}=tname{i};
...
%         plot(myinterval(index),f(myinterval(index)),'b.-')
            ...
            legend(p,tname{i},'location','west','fontsize',7);
            legend 'boxoff'; 
...
if testwithstd==2
saveas(h1,['Cells_PC_lambda_test_mean']);
saveas(h1,['Cells_PC_lambda_test_mean.png']);
end
