...
% clear all 
% real=load('../NullModel/MakeListColumnarStructurePrediction/DT_E185_nuclei/centroid_and_surface_cells.mat');
% 
...
[LCC,LCC1]=readClusterFile('../NullModel/Realization_medium/Random_cluster_1.dat');
...
function alignment(C)
    ...
    mu=mean(C);
    d = sqrt(diag(val));
    factor=2; quiver3(mu(1),mu(2),mu(3),factor*vec(1,1),factor*vec(2,1),factor*vec(3,1),d(1),'b','LineWidth',10);
    factor=2; quiver3(mu(1),mu(2),mu(3),factor*vec(1,2),factor*vec(2,2),factor*vec(3,2),d(2),'g','LineWidth',5);
    factor=2; quiver3(mu(1),mu(2),mu(3),factor*vec(1,3),factor*vec(2,3),factor*vec(3,3),d(3),'m','LineWidth',2);
