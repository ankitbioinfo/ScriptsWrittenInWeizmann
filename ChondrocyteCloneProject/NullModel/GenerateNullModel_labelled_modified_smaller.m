...
load('MakeListColumnarStructurePrediction/DT_E185_nuclei/centroid_and_surface_cells.mat','centroid');
cell_cutoff= load('../Tibia/MakeListColumnarStructurePrediction/cells_cutoff.dat');



radius=load('../Tibia/dataSave/ParameterForRandomModel_basedon_nuc_smaller.dat');
randomCellCutoff=cell_cutoff(:,1);
[~,LCC]=readClusterFile('../Tibia/MakeListColumnarStructurePrediction/ClusterSmallerCutoff.dat');
disp('LCC')
length(LCC)
...
dir2=strcat('Realization_smaller','/');
if ~exist([dir2],'dir')
    mkdir([dir2]);
end
    ...
    for k=1:length(radius)
        ...
        while flag
            ...
            if (R(k,1)>=0)&(R(k,1)<=1)
                 ...
                 cellid=Search_cell_in_approximity(X,Y,Z,2*radius(i,2),c,DV,ML,PDaxis);
